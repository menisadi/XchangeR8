...
class Rates:
    ...
    def __init__(self, csv_file):
        ...
        creating a pivot table of the best exchange rates
        for each currency pair, 
        and creating a directed graph of the rates using NetworkX library.
        ...
        rates_graph : networkx.DiGraph
            Directed graph of the exchange rates, 
            with the weights on the edges representing the 
    ...
    def _update_table_and_graph(self):
        """
        Update the banks_table, rates, and rates_graph attributes 
        based on the current data in the banks_table.
        ...
        banks_table : pandas DataFrame
            DataFrame containing the exchange rates data, with additional columns 
            for the best rate and best bank for each currency pair. 
            The banks_table attribute will be updated
            with the new exchange rates, 
            updating the 'best_bank' and 'best_rate' columns
        rates : pandas DataFrame
            Pivot table of the best exchange rates for each currency pair. 
            This attribute will be recreated 
            based on the updated banks_table attribute
        rates_graph : networkx.DiGraph
            Directed graph of the exchange rates, 
            with the weights on the edges representing the 
            negative logarithm of the rates. 
            This attribute will be recreated based on the updated 
        ...
        # Ignore error - in the initial run there is no "best_rate" column 
        # so we simply ignore the drop call
        ...
        # Create a pivot table from the best rates so we can use it 
        ...
        self.rates_graph = nx.from_pandas_adjacency(log_of_rates, 
    ...
    def update_graph(self, updates):
        ...
        for update in updates:
            ...
            # Assign the new rate in the corresponding row (source and target) 
            # and column (bank to be updated)
    ...
    def _path_to_list_of_dicts(self, path):
        """
        Helper method to convert the path returned by the bellman_ford algorithm 
    ...
    def max_revenue(self, query_dict):
        """
        Find the best path for converting a given amount of currency 
        from one type to another according to the current rates.
